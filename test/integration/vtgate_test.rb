...
class Vitess::ClientIntegrationTest < Minitest::Test
  def test_basic_selects_and_inserts
    ...
    transaction = vtgate.connect
    assert transaction.session
    insert_resp = vtgate.query_with_keyspace_ids(insert_sql, keyspace: 'test_keyspace', session: transaction.session)
    assert insert_resp.session
    commit      = vtgate.commit(session: insert_resp.session)

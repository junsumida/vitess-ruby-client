...
class Vitess::ClientIntegrationTest < Minitest::Test
  ...
  def setup
    ...
    vtgate.begin
    vtgate.query_with_keyspace_ids('delete from test_table', keyspace: 'test_keyspace', keyspace_ids: [0])
    vtgate.commit
  end
  ...
  def test_basic_selects_and_inserts
    ...
    initial_resp = vtgate.query_with_keyspace_ids('SELECT * FROM test_table', keyspace: 'test_keyspace', keyspace_ids: [0])
    initial_row_count = initial_resp.result.rows.count
    assert_kind_of Fixnum, initial_row_count, 'row count should be a fixnum'

    vtgate.begin
    insert_resp = vtgate.query_with_keyspace_ids(insert_sql, keyspace: 'test_keyspace', keyspace_ids: [0])
    assert insert_resp
    vtgate.commit

    select_resp = vtgate.query_with_keyspace_ids('SELECT * FROM test_table', keyspace: 'test_keyspace', keyspace_ids: [0])
  ...
  def test_rollback
    ...
    vtgate.rollback
    select_resp = vtgate.query_with_keyspace_ids('select * from test_table where msg = "hogehoge"', keyspace: 'test_keyspace', keyspace_ids: [0])
  ...
  def test_delete_queries
    vtgate = Vitess::Client.new(host:'192.168.99.100:15002', default_sharding_type: :id)
    vtgate.begin
    insert_resp = vtgate.query_with_keyspace_ids(insert_sql('foobar'), keyspace: 'test_keyspace', keyspace_ids: [0])
    assert insert_resp
    vtgate.commit

    # checking if inserted records exist
    select_resp = vtgate.query_with_keyspace_ids('select * from test_table where msg = "foobar"', keyspace: 'test_keyspace', keyspace_ids: [0])
    assert_equal 1, select_resp.result.rows.count, 'A row with foobar msg should exist in the test_table'

    vtgate.begin
    delete_resp = vtgate.query_with_keyspace_ids('delete from test_table where msg = "foobar"', keyspace: 'test_keyspace', keyspace_ids: [0])
    assert_equal 1, delete_resp.result.rows_affected, 'One row should be deleted from test_table'
    vtgate.commit

    reselect_resp = vtgate.query_with_keyspace_ids('select * from test_table where msg = "foobar"', keyspace: 'test_keyspace', keyspace_ids: [0])
  ...
  def test_update_queries
    ...
    vtgate.begin
    insert_resp = vtgate.query_with_keyspace_ids(insert_sql(msg), keyspace: 'test_keyspace', keyspace_ids: [0])
    assert_equal insert_resp.result.rows_affected, 1, 'One row should be inserted.'
    vtgate.commit

     # checking if inserted records exist
    select_resp = vtgate.query_with_keyspace_ids(select_sql(msg), keyspace: 'test_keyspace', keyspace_ids: [0])
    assert_equal 1, select_resp.result.rows.count, "A row with #{msg} msg should exist in the test_table"

    vtgate.begin
    update_resp = vtgate.query_with_keyspace_ids('update test_table set msg = "sticky_toffee"', keyspace: 'test_keyspace', keyspace_ids: [0])
    assert_equal 1, update_resp.result.rows_affected, "A msg #{msg} should be replaced with sticky_toffee"
    vtgate.commit

    select_resp = vtgate.query_with_keyspace_ids(select_sql('sticky_toffee'), keyspace: 'test_keyspace', keyspace_ids: [0])
